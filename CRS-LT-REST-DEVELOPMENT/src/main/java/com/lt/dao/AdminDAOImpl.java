...
public class AdminDAOImpl implements AdminDAOInterface
{
	...
	public boolean adminApproval(int studentId) 
	{
		...
		try 
		{
			String str = "update student set isapproved = 1 where student_id = ? and isapproved=0";
	...
	public List<Professor> getProfessor()
	{
		...
		try 
		{
			...
			String str = "select prof_id,prof_name,department,address from professor";
			PreparedStatement myStmt = conn.prepareStatement(str);
			ResultSet myRs = myStmt.executeQuery();
			
			while(myRs.next())
			{
				int profId=myRs.getInt(1);
				String profName = myRs.getString(2); 
				String department = myRs.getString(3);
				String address = myRs.getString(4);
				
				profList.add(new Professor(profId,profName,department,address));	
			}
		} 
        catch (SQLException ex)
        {
