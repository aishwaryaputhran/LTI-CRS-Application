...
public class RegisteredCourseDAOImpl  implements RegisteredCourseDAOInterface
{
    ...
    */
	...
	@Override
	public List<RegisteredCourse> viewAllRegisteredCourses() {
		...
		try 
		{
			...
			String str = "select course_code, course_name, student_id, course_fee from registered_course";
			...
			while(resultSet.next())
			{
				...
				double courseFee = resultSet.getDouble("course_fee");
				RegisteredCourse obj = new RegisteredCourse(courseCode, courseName, studentId, courseFee);
				availRegCourseList.add(obj);
			}
